...
// 스폰 간섭 없이 수동 배치로 테스트하기 위한 준비
function quiet(g) { g.spawnTimer = 9999; g.falling.length = 0; }
...
console.log('\n[4] 콤보: 3초 안에 연속 배출 → x1.5(5), x2(10) / 끊기면 리셋');
{
  const g = createGame(1);
  quiet(g);
  for (let i = 0; i < 10; i++) g.lanes[1].queue.push({ id: i, size: 1 });
  g.lanes[1].fill = 10;
  run(g, 5); // S 10개 × 0.4s = 4초 — 콤보 윈도(3s) 내 연속
...
function runBot(seed, decide, maxT) {
  const g = createGame(seed);
  let decideTimer = 0;
...
console.log('\n[8] 밸런스 시뮬레이션 (20시드)');
{
  const stats = (bot) => {
    const rs = [];
    for (let s = 1; s <= 20; s++) rs.push(runBot(s * 1000 + 7, bot, 600));
    rs.sort((a, b) => a.t - b.t);
    return { med: rs[10], max: rs[rs.length - 1], maxScore: Math.max(...rs.map((r) => r.score)) };
  };
  const bi = stats(idle), bg = stats(greedy), bs = stats(smart);
  ...
  assert(bi.med.t < 60, '방치하면 1분 내 사망 (압박 존재)');
  assert(bg.med.t < 300, '탐욕 전략은 5분을 못 버틴다 (자명한 전략 차단)');
  assert(bs.med.t > 45, '스마트 전략은 45초 이상 (불공정 아님)');
  assert(bs.max.t < 600, '아무도 영생 불가 (램프가 결국 이긴다)');
  assert(bs.med.t > bg.med.t * 0.9, '머리 쓰는 쪽이 손해 보지 않는다');
  console.log(`\n  → 커피 이스터에그 기준 참고: 스마트봇 최고 ${bs.maxScore}점 (사람 고수는 콤보/rescue로 이를 상회 가능)`);
}
